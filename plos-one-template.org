...
This source code block initializes Ebib and binds =C-c b= to the command
that inserts the cite: link.  It instructs Ebib to look for the BibTeX
database, =local.bib=, in the current directory.

The last line defines the command that inserts the cite: link.  It
will prompt once for a description of the citation. The description is
for convenience only, so the citation looks reasonable in the Org mode
buffer.  It is not used during export.
