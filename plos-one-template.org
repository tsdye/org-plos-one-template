...
** DONE Add ebib stuff
   :LOGBOOK:
   - State "DONE"       from "TODO"       [2014-03-02 Sun 09:49]
   :END:

** DONE Make github repo
   :LOGBOOK:
   - State "DONE"       from "TODO"       [2014-03-02 Sun 08:37]
   :END:
* Instructions for Your /PLOS ONE/ Article                         :noexport:

This file provides a template to export an Org mode file to a LaTeX
file that meets the specifications of the open-access journal [[http://www.plosone.org][/PLOS
ONE/]]. It features:
...
The following lines initialize the Org mode LaTeX exporter, indicate
the name of the asynchronous export initialization file, and then
specify that asynchronous export should be used and that the export
process should leave data behind in case there are problems.
...
The following line provides compatibility across a recent Org mode
...
The following line executes a source code block that defines an Org
mode link that functions to insert a bibliographic reference.
...
** Checklist [0/9]                                 
  ...
  - [ ] Optionally, export the supporting information document
  - [ ] Export the article
